            ...
            function(data) {
                if (data.length > 0 && data[0]['CountMarks'] !== undefined) {
                    for (i = 0; i < data.length; i++) {
                        ...
                        str += "<div class=\"media\"><div class=\"col-lg-7\"><div class=\"media-body\"><div class=\"title\"><a href=\"https://www.merriam-webster.com/dictionary/" + data[i]['Word'] + "\" target=\"_blank\" id=\"each_word\">" + data[i]['Word'] + "</a></div><p id=\"means\"><span>• " + data[i]['Means'] + "</span></p><p>" + notice_text + "</p></div></div><div class=\"col-lg-offset-7\"><button onclick=\"ShowMeans(this)\" type=\"button\" class=\"btn btn-info btn-lg" + disable_tag + "\">显示意思</button> <button value=\"" + data[i]['Id'] + "\" onclick=\"MarkWord(this)\" type=\"button\" class=\"btn btn-warning btn-lg\">没记住+1</button> <button value=\"" + data[i]['Id'] + "\" onclick=\"DeleteWord(this)\" type=\"button\" class=\"btn btn-danger btn-sm\">删除</button></div></div><div class=\"divide mar-top-5\"></div>";
                    }
                } else {
                    for (i = 0; i < data.length; i++) {
